        ...
        for unit_id in sorted(sensor_data.keys()):
            ...
            if rul_list:
                ...
                with cols[0].expander("RUL Trend"):
                    # Create Plotly graph for RUL trend
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(
                        x=df_rul['timestamp'], y=df_rul['rul'], mode='lines', name="RUL"))
                    fig.update_layout(title="RUL Evolution",
                                      xaxis_title="Timestamp",
                                      yaxis_title="Predicted RUL",
                                      showlegend=False)
                    st.plotly_chart(
                        fig, use_container_width=True, key=f"rul-{unit_id}")
            ...
            if sensor_list:
                ...
                with chart_cols[0]:
                    ...
                    # Create Plotly chart for sensors
                    fig_sensors = go.Figure()
                    for sensor in SENSORS:
                        fig_sensors.add_trace(go.Scatter(
                            x=df_sensors['timestamp'], y=df_sensors[sensor], mode='lines', name=sensor))
                    fig_sensors.update_layout(title="Sensor Readings",
                                              xaxis_title="Timestamp",
                                              yaxis_title="Sensor Value",
                                              showlegend=True)
                    st.plotly_chart(
                        fig_sensors, use_container_width=True, key=f"sensors-{unit_id}")

                with chart_cols[1]:
                    st.subheader("Operational Settings")
                    # Create Plotly chart for operational settings
                    fig_op_settings = go.Figure()
                    for setting in OP_SETTINGS:
                        fig_op_settings.add_trace(go.Scatter(
                            x=df_sensors['timestamp'], y=df_sensors[setting], mode='lines', name=setting))
                    fig_op_settings.update_layout(title="Operational Settings",
                                                  xaxis_title="Timestamp",
                                                  yaxis_title="Setting Value",
                                                  showlegend=True)
                    st.plotly_chart(
                        fig_op_settings, use_container_width=True, key=f"opsettings-{unit_id}")
            st.markdown("---")
